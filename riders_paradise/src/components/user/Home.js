import React from "react";
import "../../styles/Home.css";
import img from "../../assets/bike.jpg";
import video from "../../assets/video.mp4";
import { Link } from "react-router-dom";
import { Button } from "bootstrap";

function Home() {
  return (
    <div class="body-homepage">
      <div className="video-container">
        <video src={video} autoPlay loop />
        <h1 className="h-home">Adventure Awaits</h1>
        <p className="p-home">What are you waiting for??</p>
      </div>
      <div className="cards-home">
        <ul className="carditems-home">
        <Link className="link-home">
          <figure className="figure-home">
            <img src={img} alt="bike1" className="img-home"/>
          </figure>
        </Link>

        <Link className="link-home">
          <figure className="figure-home">
            <img src={img} alt="bike2" className="img-home"/>
          </figure>
        </Link>
        </ul>

        <ul className="carditems-home">
        <Link className="link-home">
          <figure className="figure-home">
            <img src={img} alt="bike3" className="img-home"/>
          </figure>
        </Link>

        <Link className="link-home">
          <figure className="figure-home">
            <img src={img} alt="bike4" className="img-home"/>
          </figure>
        </Link>
        </ul>
      </div>
    </div>
  );
}

export default Home;
